...
Let fm (x : (m.-tuple {: n.-tuple A})) := extension f x.
...
have: extension f \o (flatten \o map (@tval m _)) =1 extension fm.
...
pose fm1 (x :[finType of ((m''.+1).-tuple {: n.-tuple A})]) := extension f x.
pose Xm1 := mkRvar ((P `^ n) `^ (m''.+1)) (INR \o size \o fm1 ).
pose fm2 (x :[finType of ((m''.+2).-tuple {: n.-tuple A})]) := extension f x.
...
pose fm (x : ((m eps).-tuple {: n.-tuple A})) := extension f x.
